...
class SplashViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if let token = OAuth2TokenStorage().token {
            ...
            self.profileImageService.fetchProfileImageURL(username: ProfileService.shared.getProfile()?.username ?? "") { [weak self] result in
        ...
        OAuth2Service.shared.fetchAuthToken(code) { [weak self ] result in
            ...
            DispatchQueue.main.async {
                ...
                case .success(let accessToken):
                    ...
                    ProfileService.shared.fetchProfile(accessToken)
                    self.profileImageService.fetchProfileImageURL(username: ProfileService.shared.getProfile()?.username ?? "") { result in
